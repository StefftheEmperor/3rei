...
class Domain extends \Db\Classes\AbstractModel implements \Db\Interfaces\Model {
	...
	public static function factory_by_host(\Db\Classes\Mysql\Connection $connection, $host)
	{
		...
		$filter = \Db\Classes\Filter::factory(\Db\Classes\Expression\Row::factory('host'), '=', \Db\Classes\Expression\Value::factory($host));
		$result = $connection->get_model_reflection('\Request\Model\Rewrite\Table')->factory($domain->get_table_name())->filter($filter)->get_one();
